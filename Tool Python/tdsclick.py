...
def login_tds(token):
	try:
		...
		if 'success' in r:
			os.system('clear')
			print(Colors.green + f"Code này mình chỉ update thêm chứ ko phải của tui nhé !!!")
	...
	if check_log == 'success':
		if cache == 'old':
			...
			if choice == 1:
				...
				os.remove('token_tds.txt')

		else:
			f = open(f'token_tds.txt', 'w')
			f.write(f'{token_tds}')
			f.close()
			break
	else:
...
if check_log == 'success':
	#Nhập user tiktok
	while True:
		id_tiktok = Write.Input("Nhập ID tiktok muốn chạy: ", Colors.green_to_yellow, interval=0.0025)
		for _ in range(3):
	...
	while True:
		...
		try:
			...
			if choice ==1:
				now = int(Write.Input("Mở qua app <> 1 Tiktok <> 2 Tiktok Now: ", Colors.green_to_yellow, interval=0.0025))
			else:
				now=False
			if choice in [1,2]:
				break
	...
	while True:
		try:
			...
			if delay > -1:
				...
				os.system('clear')
				print(Colors.red + f"Delay tối thiểu là 0\n")
		except:
			os.system('clear')
	...
	while True:
		...
		if isinstance(list_job, dict) == True:
			for job in list_job['data']:
				...
				lk=f"tiktoknow://user/profile?user_id={idd}"
				if now==2 and choice ==1:
					os.system(f'termux-open-url {lk}')
				else:
