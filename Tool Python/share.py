...
\n'''
        print(Colorate.Vertical(Colors.DynamicMIX((Col.light_green, Col.light_gray)), Center.XCenter(title+'\n   Token: '+self.total)) + Colorate.Vertical(Colors.DynamicMIX((Col.light_red, Col.light_blue)), Center.XCenter(banner)))
        if self.total == '0':
    ...
    def share(self, id_post, token):
        ...
        if 'id' in response:
            ...
            print(self.format_print("*",f"{dt_now.strftime('%H:%M:%S')}: SHARE POST FAILED!"))
    def run_share(self):
        while True:
            main.banner()
            try: 
                id_post = input(self.format_input("!",f"Nhập ID Bài Viết: "))
                threa = int(input(self.format_input("!",f"INPUT THREAD: ")))
