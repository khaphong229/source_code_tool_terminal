...
if "success" in datch:
    print("\033[37;1mĐặt cấu hình\033[32;1m",name,"<=>",ten,"\033[32;1mthành công")
else:
    print("\033[37;1mĐặt cấu hình\033[32;1m",name,"<=>",ten,"\033[32;1mthất bại")
...
xtoken=ss.head(f"https://www.tiktok.com/api/comment/publish/",headers=head_x).headers['x-ware-csrf-token'].split(',')[1]


def nhan_cmt(idd,api):
...
while True:
    while True:
        get_cmt=ss.get(f"https://traodoisub.com/api/?fields=tiktok_comment&access_token={api}").json()
        if get_cmt==[]:
            print("Hết job ...",end=" \r")
            break
        else:
            try:
                for x in get_cmt:
                    ...
                    try:
                        response = ss.post('https://www.tiktok.com/api/comment/publish/', params=params, headers=headers).json()
                        check=response['status_msg']
